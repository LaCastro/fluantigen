...
for(n in N) {
    startCmd <- "java -jar trackAntigen17.jar"
    dirName <- paste0(" ", type.sim, "_" , n)
    grbCmd <- " -XX:+UseSerialGC"
    memoryCmd <- " -Xmx5G "
    mainCmd <- "Mutantigen"
    
    #full_cmd <- paste0(startCmd, dirName, nxtCmd, memoryCmd, mainCmd)
    full_cmd <- paste0(startCmd, dirName, grbCmd, memoryCmd, mainCmd)

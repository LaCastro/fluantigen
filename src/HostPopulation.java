...
public class HostPopulation {
	...
	public HostPopulation(int d) {
		...
		for (int i = 0; i < initialS; i++) {
			Host h = new Host();
			susceptibles.add(h);  //adding to the array
		}
 ...
 */       
	...
	//	resetCases();
		if (Parameters.swapDemography) {
			...
			decline();
                ...
                computeMeanLoad(); // this reset the value in Parameters 
                getInfectionsByClass(); // returns an array of lists, positions 
                                        // have the names of viruses (parent id) , positions refer to # deleterious mutations
        ...
        public void getInfectionsByClass() {
            ...
            infectionsByClass = new ArrayList<ArrayList<Host>>(); // creates an array of hosts (it says new...was this here before in the population)
            for (int k = 0; k <= max; k++) {
	...
	public void contactByMutClass() {
                ...
                for (int k = 0; k < maxK; k++) {
                    ...
                    double betaK = Parameters.beta * Math.pow((1-Parameters.mutCost), k); // calculate beta based on deleterious load
                    ...
                    for (int i = 0; i < contacts; i++) {
                            if (getS()>0 && getI()>0) {
                                    ...
                                    Host testH = infectionsByClass.get(k).get(randomInK); // get random host from k class  
